function regret_plot = IKGwSAA(density_type,cost_type,parpool_n,d,C,sample_size_ratio)
...
x_indep_Cov_X_V_MB = cell(1,M); 
for i = 1:M
    x_indep_Cov_X_V_MB{i} = zeros(J3,N0);
end
x_indep_mu_prior_MB = mu_prior(-M,X3);
x_indep_mu_MB = x_indep_mu_prior_MB;
...
t_SGA_start = tic;
[a,v,logIKGmax] = SAA(IKG_pra,V,Vidx,Cov_Noise_Inv,X,v0n,x_indep_mu,[],x_indep_mu_MB,[],X3);
t_SGA = toc(t_SGA_start);
...
while consumption <= C
    ...
    % update x_indep_MB
    x_indep_Cov_X_V_MB{a}(:,idx) = cov_vector(v, X3, theta(:,1,a), tau2(:,1,a))';
    x_indep_mu_MB(:,a) = x_indep_mu_prior_MB(:,a) +  x_indep_Cov_X_V_MB{a}(:,1:idx) * Cov_Noise_Inv_y_mu(1,1:idx,a)';     
    ...
    if idx >= N0cell(a)
        ...
        store_temp = x_indep_Cov_X_V_MB{a};
        x_indep_Cov_X_V_MB{a} = zeros(J,N0cell(a));
        x_indep_Cov_X_V_MB{a}(:,1:idx) = store_temp;        
    end
    ...
    % decide the sample location using IKG
    t_SGA_start = tic;
    [a,v,logIKGmax] = SAA(IKG_pra,V,Vidx,Cov_Noise_Inv,X,v0n,x_indep_mu,x_indep_Cov_X_V,x_indep_mu_MB,x_indep_Cov_X_V_MB,X3);
    t_SGA_last = toc(t_SGA_start);
    t_SGA = t_SGA + t_SGA_last;    
    consumption = consumption + cost(a,v,cost_type);    
end
regret_cost_L{ell} = regret_cost(:,1:n);
time_L(ell) = t_SGA - t_SGA_last; 
end
% delete(gcp)
...
% save data
save(['IKGwSAA_' num2str(d) 'D_density' num2str(density_type) '_cost' num2str(cost_type) '.mat'],'regret_cost_L','regret_plot','time_L');