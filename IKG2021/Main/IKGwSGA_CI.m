function regret_plot = IKGwSGA_CI(density_type,cost_type,parpool_n,d,C,SGA_m,SGA_K,SGA_b1,SGA_b2)
...
IKG_pra.cost_type = 1; % ignore the cost when solving IKG
...
[a,v,logIKGmax] = SGA_ave_bat(IKG_pra,V,Vidx,Cov_Noise_Inv,Cov_Noise_Inv_y_mu,X,v0n,x_indep_mu,[]);
consumption = cost(a,v,cost_type); % actual sampling consumption follows c_i(x)
...
while consumption <= C
    ...
    % decide the sample location using IKG
    tic
    [a,v,logIKGmax] = SGA_ave_bat(IKG_pra,V,Vidx,Cov_Noise_Inv,Cov_Noise_Inv_y_mu,X,v0n,x_indep_mu,x_indep_Cov_X_V);
...
% save data
save(['IKGwSGA_CI_' num2str(d) 'D_density' num2str(density_type) '_cost' num2str(cost_type) '.mat'],'regret_cost_L','regret_plot');