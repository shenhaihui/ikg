function regret_plot = BSE(density_type,cost_type,parpool_n,d,C,bin_M)
...
Ybar = zeros(bin_N, M);
YbarUpdate = zeros(bin_N, M);
a_bin = zeros(bin_N, 1);
tau_bin = ones(bin_N, 1);
S_bin = cell(bin_N, 1);
for i = 1: bin_N
    S_bin{i} = 1:1:M;
end
...
n = 1;
v = rand(d,1)*10;
bin_index = find_bin_index(v,bin_M);
[a_bin(bin_index), S_bin{bin_index}, tau_bin(bin_index), Ybar(bin_index,:)] ...
    = SE(a_bin(bin_index), S_bin{bin_index}, tau_bin(bin_index), Ybar(bin_index,:), YbarUpdate(bin_index,:), C/bin_N);
a = a_bin(bin_index);
...
while consumption <= C
    ...
    % sperately store y in YbarUpdate
    % roughly normalize it to [0,1]
    YbarUpdate(bin_index,a) = 1/tau_bin(bin_index) * ((tau_bin(bin_index)-1)*YbarUpdate(bin_index,a) + (yn(1,idx,a)/(1.5^(d-1)*2)+0.5) );
...
% save data
save(['BSE_' num2str(d) 'D_density' num2str(density_type) '_cost' num2str(cost_type) '.mat'],'regret_cost_L','regret_plot');